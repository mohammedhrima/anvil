...
void walk(const std::string &root, const std::function<void(const std::string &)> &cb)
{
    ...
    while (dirent *e = readdir(d)) {
        ...
        if (S_ISDIR(st.st_mode)) {
            if (std::strcmp(e->d_name, "build") == 0)
                continue;
            walk(full, cb);
...
static std::string read_quoted(const std::string &s, size_t &i, const std::string &path)
{
    ...
    if (i >= s.size())
        die("unterminated string in " + path);
    i++;
    return out;
}
    ...
    while (i < s.size()) {
        ...
        if (s[i] == '"') {
            ...
            while (i < s.size()) {
                ...
                if (s[i] != '"')
                    die("expected string in array " + key);
...
static void load()
{
    ...
    if (path.empty())
        die("no anvil.toml in current directory (pass -f <path> to override)");
