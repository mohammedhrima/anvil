...
int cmd_build(const std::vector<std::string> &args)
{
    ...
    for (const auto &s : cfg.compile)
        argv.push_back(resolve(cfg.source + "/" + s));
    if (want_release)
        argv.insert(argv.end(), cfg.release.begin(), cfg.release.end());
    else {
                       ...
                       const std::vector<std::string> &patterns)
{
    ...
    for (const auto &p : patterns) {
        ...
        if (ura_file == full)
            return true;
        if (ura_file.rfind(full, 0) == 0 && full.size() < ura_file.size() && (full.back() == '/' || ura_file[full.size()] == '/'))
            return true;
    }
    return false;
}
                               ...
                               const std::string &result_path)
{
    ...
    std::string root_with_slash = tests_root + "/";
    if (rel.rfind(root_with_slash, 0) == 0)
        rel = rel.substr(root_with_slash.size());
    ...
    bool is_negative = !errors_root.empty() && ura_file.rfind(errors_root, 0) == 0;
    if (is_negative) {
        int rc = run_compiler_silent(ura_bin, ura_file);
        if (rc == 0)
            write_result(result_path, "FAIL", rel, "expected parse error, none fired");
        else if (rc >= 128)
            write_result(result_path, "FAIL", rel, "crashed (signal)");
        else
            write_result(result_path, "PASS", rel, "");
        _exit(0);
    }
    ...
    if (a == b)
        ...
        write_result(result_path, "FAIL", rel, "IR mismatch");
    _exit(0);
}
...
int cmd_test(const std::vector<std::string> &args)
{
    ...
    auto collect = [&](const std::string &p) {
        if (p.size() >= 4 && p.substr(p.size() - 4) == ".ura")
            files.push_back(p);
    };
    if (args.empty()) {
    ...
    auto label = [&](std::size_t idx) {
        ...
        if (rel.rfind(rws, 0) == 0)
            rel = rel.substr(rws.size());
        if (rel.size() >= 4 && rel.substr(rel.size() - 4) == ".ura")
            rel = rel.substr(0, rel.size() - 4);
        return rel;
    };
    ...
    while (next_print < files.size()) {
        ...
        if (next_print < files.size() && !running.empty())
            reap_one();
        else if (next_print < files.size())
            break; // nothing running, nothing to launch → bail to flush
    }
    while (!running.empty())
        reap_one();
    ...
    for (std::size_t i = 0; i < files.size(); i++)
        unlink((tdir + "/" + std::to_string(i)).c_str());
    rmdir(tdir.c_str());
    ...
    if (s)
        printf(ANSI_YELLOW "Skipped: %d" ANSI_RESET "\n", s);
...
int cmd_install(const std::vector<std::string> &)
{
    ...
    if (os == "linux-apt") {
        std::string cmd = "sudo apt-get update && sudo apt-get install -y llvm-" + vstr + " clang-" + vstr + " llvm-" + vstr + "-tools clang-format";
        int rc = run_sh(cmd);
        if (rc != 0)
            return rc;
    ...
    fprintf(stderr, ANSI_RED "install: unknown platform %s" ANSI_RESET ". Install llvm-%s, clang, and clang-format manually.\n",
            os.c_str(), vstr.c_str());
    return 1;
}
...
int cmd_copy(const std::vector<std::string> &args)
{
    ...
    if (!ura_src.empty() && ura_src[0] != '/')
        ura_src = resolve(cfg.source) + "/" + ura_src;
    if (!is_file(ura_src)) {
...
int cmd_update_tests(const std::vector<std::string> &)
{
    ...
    walk(tests, [&](const std::string &p) {
        ...
        if (!errors_root.empty() && p.rfind(errors_root, 0) == 0)
            return;
        if (is_skipped(p, tests, cfg.skip))
            return;
        files.push_back(p);
    });
    std::sort(files.begin(), files.end());
...
int cmd_update_projects(const std::vector<std::string> &)
{
    ...
    for (const auto &path : files) {
        ...
        std::string rel = path;
        if (rel.rfind(tests_dir + "/", 0) == 0)
            rel = rel.substr(tests_dir.size() + 1);
        if (rel.size() >= 4 && rel.substr(rel.size() - 4) == ".ura")
            rel = rel.substr(0, rel.size() - 4);
                           ...
                           const std::string &ura_lib_dir)
{
    ...
    std::string cmd =
        ...
        "cp '" +
        ura_bin + "' \"$stage/bin/ura\"; "
                  "cp -R '" +
        ura_lib_dir + "' \"$stage/share/ura/lib\"; "
...
int cmd_examples(const std::vector<std::string> &)
{
    ...
    if (!f) {
        fprintf(stderr, ANSI_RED "examples: cannot open %s" ANSI_RESET "\n", out.c_str());
        return 1;
    }
...
bool image_exists()
{
    CaptureResult r = run_capture({"docker", "image", "inspect", IMAGE, "--format", "{{.Id}}"});
    return r.code == 0;
}
...
int cmd_shell(const std::vector<std::string> &)
{
    ...
    std::string cmd = "docker run --platform linux/amd64 --rm -it -v '" + project_root() + ":/workspace' " + IMAGE;
    return run_sh(cmd);
}
